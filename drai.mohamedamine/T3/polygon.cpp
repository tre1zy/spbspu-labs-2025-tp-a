#include <iostream>
#include <fstream>
#include <sstream>
#include <algorithm>
#include <numeric>
#include <cmath>
#include <iomanip>
#include <functional>

#include "polygon.hpp"

static const std::size_t RECT_VERTICES = 4;

Point operator-(const Point& a, const Point& b) {
  return Point(a.x - b.x, a.y - b.y);
}

int dot(const Point& a, const Point& b) {
  return a.x * b.x + a.y * b.y;
}

int cross(const Point& a, const Point& b) {
  return a.x * b.y - a.y * b.x;
}

double compute_area(const Polygon& poly) {
  const auto& pts = poly.points;
  if (pts.size() < 3) return 0.0;

  long long area2 = std::inner_product(
    pts.begin(), pts.end() - 1,
    pts.begin() + 1, 0LL,
    std::plus<>(),
    [](const Point& a, const Point& b) {
      return static_cast<long long>(a.x) * b.y - static_cast<long long>(a.y) * b.x;
    });

  area2 += static_cast<long long>(pts.back().x) * pts.front().y - static_cast<long long>(pts.back().y) * pts.front().x;
  return std::abs(area2) / 2.0;
}

bool is_rectangle(const Polygon& poly) {
  if (poly.points.size() != RECT_VERTICES) return false;
  const auto& p = poly.points;

  std::array<Point, 4> v = {
    p[1] - p[0],
    p[2] - p[1],
    p[3] - p[2],
    p[0] - p[3]
  };

  if (!(dot(v[0], v[1]) == 0 && dot(v[1], v[2]) == 0 && dot(v[2], v[3]) == 0 && dot(v[3], v[0]) == 0)) return false;

  std::array<long long, 4> lengths;
  std::transform(v.begin(), v.end(), lengths.begin(), [](const Point& vec) {
    return static_cast<long long>(vec.x) * vec.x + static_cast<long long>(vec.y) * vec.y;
  });

  return lengths[0] == lengths[2] && lengths[1] == lengths[3];
}

long long orient(const Point& p, const Point& q, const Point& r) {
  return static_cast<long long>(q.x - p.x) * (r.y - p.y) - static_cast<long long>(q.y - p.y) * (r.x - p.x);
}

bool on_segment(const Point& p, const Point& q, const Point& r) {
  return q.x >= std::min(p.x, r.x) && q.x <= std::max(p.x, r.x) &&
         q.y >= std::min(p.y, r.y) && q.y <= std::max(p.y, r.y);
}

bool seg_intersect(const Point& p1, const Point& q1, const Point& p2, const Point& q2) {
  long long o1 = orient(p1, q1, p2);
  long long o2 = orient(p1, q1, q2);
  long long o3 = orient(p2, q2, p1);
  long long o4 = orient(p2, q2, q1);

  return (o1 * o2 < 0 && o3 * o4 < 0) ||
         (o1 == 0 && on_segment(p1, p2, q1)) ||
         (o2 == 0 && on_segment(p1, q2, q1)) ||
         (o3 == 0 && on_segment(p2, p1, q2)) ||
         (o4 == 0 && on_segment(p2, q1, q2));
}

bool polygons_intersect(const Polygon& a, const Polygon& b) {
  const auto& pa = a.points;
  const auto& pb = b.points;

  bool edge_intersection = std::any_of(pa.begin(), pa.end(), [&](const Point& p, size_t i = 0) mutable {
    const Point& a1 = pa[i];
    const Point& a2 = pa[(i + 1) % pa.size()];
    ++i;
    return std::any_of(pb.begin(), pb.end(), [&](const Point& q, size_t j = 0) mutable {
      const Point& b1 = pb[j];
      const Point& b2 = pb[(j + 1) % pb.size()];
      ++j;
      return seg_intersect(a1, a2, b1, b2);
    });
  });

  if (edge_intersection) return true;

  return !pa.empty() && !pb.empty() && (point_in_polygon(pa[0], b) || point_in_polygon(pb[0], a));
}

bool point_in_polygon(const Point& pt, const Polygon& poly) {
  const auto& p = poly.points;
  if (p.size() < 3) return false;

  return std::accumulate(p.begin(), p.end(), std::make_pair(false, p.size() - 1), [&](std::pair<bool, std::size_t> acc, const Point& curr) {
    std::size_t i = acc.second;
    bool inside = acc.first;
    bool cond = (curr.y > pt.y) != (p[i].y > pt.y);
    bool intersect = cond &&
      (pt.x < static_cast<long double>(p[i].x - curr.x) * (pt.y - curr.y) / (p[i].y - curr.y) + curr.x);
    return {inside ^ intersect, i + 1};
  }).first;
}

bool parse_polygon(const std::string& str, Polygon& poly) {
  std::istringstream iss(str);
  int n;
  if (!(iss >> n) || n <= 0) return false;

  poly.points.clear();
  poly.points.reserve(n);
  return std::all_of(
    std::istream_iterator<char>(iss),
    std::istream_iterator<char>(),
    [&](char ch) {
      if (ch != '(') return false;
      Point pt;
      if (!(iss >> pt.x)) return false;
      if (!(iss >> ch) || ch != ';') return false;
      if (!(iss >> pt.y)) return false;
      if (!(iss >> ch) || ch != ')') return false;
      poly.points.push_back(pt);
      return true;
    }) && poly.points.size() == static_cast<std::size_t>(n);
}
