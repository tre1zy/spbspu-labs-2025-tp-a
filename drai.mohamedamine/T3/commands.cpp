  ...
  void execute_command(const std::string& command, const std::vector<std::string>& args, std::vector<Polygon>& polygons) {
    ...
    try {
      if (command == "AREA") {
        ...
        if (args.empty() || (args[0] != "AREA" && args[0] != "VERTEXES")) {
          std::cout << "<INVALID COMMAND>\n";
          return;
        }

        if (args[0] == "AREA") {
          double max_area = polygons.empty() ? 0.0 :
          compute_area(*std::max_element(polygons.begin(), polygons.end(),
            [](const Polygon& a, const Polygon& b) {
              return compute_area(a) < compute_area(b);
            }));
          std::cout << max_area << '\n';
        } else if (args[0] == "VERTEXES") {
          size_t max_vertexes = polygons.empty() ? 0 :
        ...
        if (!parse_polygon(poly_str, param) || param.vertexCount() < 3) {
          std::cout << "<INVALID COMMAND>\n";
          return;
        }
                             ...
                             std::vector<Polygon>& polygons)
    {
        ...
        static const std::vector<std::string> known_commands = {
         "AREA", "MAX", "MIN", "COUNT", "INTERSECTIONS", "RMECHO",
         "ECHO", "RECTS", "PERMS", "SAME", "INFRAME", "RIGHTSHAPES", "MAXSEQ"
        };
        auto next_cmd = std::find_if(it, end,
            [](const std::string& s) {
                return std::find(known_commands.begin(), known_commands.end(), s) != known_commands.end();
            });
