...
struct DictEntryPrinter
{
  ...
  int operator()(const tree_t::value_type& entry) const
  {
      ...
      std::transform(entry.second.begin(), entry.second.end(), std::back_inserter(dummy), TranslationPrinter{ out });
      out << "\n";
      return 0;
  }
};
...
struct DictSaver
{
  ...
  int operator()(const dict_t::value_type& dict_pair) const
  {
      out << dict_pair.first << "\n";
      std::vector<int> dummy;
      std::transform(dict_pair.second.begin(), dict_pair.second.end(),
          std::back_inserter(dummy), DictEntryCopier{ out });
      out << "\n";
      return 0;
  }
};
...
struct ComplementPredicate
{
  ...
  {
      return dict.find(entry.first) == dict.end();
  }
};
...
struct LongestPredicate
{
  ...
  bool operator()(const tree_t::value_type& entry) const
  {
      return entry.first.size() == max_len;
  }
};
...
struct MeaningfulPredicate
{
  ...
  bool operator()(const tree_t::value_type& entry) const
  {
      return entry.second.size() == max_count;
  }
};

struct MaxLenFinder
{
  const tree_t::value_type* max_entry;
  MaxLenFinder() : max_entry(nullptr) {}
  void operator()(const tree_t::value_type& entry)
  {
      if (!max_entry || entry.first.size() > max_entry->first.size())
      {
          max_entry = &entry;
      }
  }
...
{
  const tree_t::value_type* max_entry;
  MaxCountFinder() : max_entry(nullptr) {}
  void operator()(const tree_t::value_type& entry)
  {
      if (!max_entry || entry.second.size() > max_entry->second.size())
      {
          max_entry = &entry;
      }
  }
...
void maxlen(std::istream& in, std::ostream& out, const dict_t& dicts)
{
  ...
  out << dict_name << " " << finder.max_entry->first << " " << finder.max_entry->first.size() << "\n";
}
...
void alfrange(std::istream& in, std::ostream& out, const dict_t& dicts)
{
  ...
  std::vector<std::string> words;
   std::transform(dict_it->second.begin(), dict_it->second.end(),
...
void prefix(std::istream& in, std::ostream& out, const dict_t& dicts)
{
  ...
  std::copy_if(words.begin(), words.end(), std::back_inserter(matched),
      PrefixChecker{ prefix_str });
...
void longest(std::istream& in, dict_t& dicts)
{
  ...
  for (const auto& name : dict_names)
  {
      ...
      size_t max_len = finder.max_entry->first.size();
...
void meancount(std::istream& in, std::ostream& out, const dict_t& dicts)
{
  ...
  size_t count = std::accumulate(
      word_it->second.begin(), word_it->second.end(),
      0, TranslationCounter{});
...
void meaningful(std::istream& in, dict_t& dicts)
{
  ...
  for (const auto& name : dict_names)
  {
      ...
      size_t max_count = finder.max_entry->second.size();
