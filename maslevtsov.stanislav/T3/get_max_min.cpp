...
namespace {
  void get_max_area(const std::vector< maslevtsov::Polygon >& polygons, std::ostream& out)
  {
  ...
  void get_max_vertexes(const std::vector< maslevtsov::Polygon >& polygons, std::ostream& out)
  {
    out << std::max_element(polygons.cbegin(), polygons.cend(), maslevtsov::compare_vertex_num_less)->points.size();
  }

  void get_min_area(const std::vector< maslevtsov::Polygon >& polygons, std::ostream& out)
  {
    std::vector< double > areas;
    std::transform(polygons.cbegin(), polygons.cend(), std::back_inserter(areas), maslevtsov::get_polygon_area);
    maslevtsov::IOFmtGuard guard(out);
  ...
  void get_min_vertexes(const std::vector< maslevtsov::Polygon >& polygons, std::ostream& out)
  {
...
void maslevtsov::get_max(const std::vector< Polygon >& polygons, std::istream& in, std::ostream& out)
{
  ...
  subcommands["AREA"] = std::bind(get_max_area, std::cref(polygons), _1);
  subcommands["VERTEXES"] = std::bind(get_max_vertexes, std::cref(polygons), _1);
...
void maslevtsov::get_min(const std::vector< Polygon >& polygons, std::istream& in, std::ostream& out)
{
  ...
  std::map< std::string, std::function< void(std::ostream&) > > subcommands;
  using namespace std::placeholders;
  subcommands["AREA"] = std::bind(get_min_area, std::cref(polygons), _1);
  subcommands["VERTEXES"] = std::bind(get_min_vertexes, std::cref(polygons), _1);
