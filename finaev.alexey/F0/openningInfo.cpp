...
std::istream& finaev::operator>>(std::istream& in, DebutName& n)
{
  std::istream::sentry sentry(in);
  if (!sentry)
  {
  ...
  if (in)
  {
    n.name = std::move(temp);
  }
  return in;
}

std::istream& finaev::operator>>(std::istream& in, DebutDescription& d)
{
  std::istream::sentry sentry(in);
  if (!sentry)
  {
