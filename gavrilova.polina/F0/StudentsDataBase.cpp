...
namespace gavrilova {
  ...
  namespace {
    ...
    struct IsGradeInPeriod {
      ...
      bool operator()(const std::pair< date::Date, int >& grade) const
      {
    ...
    struct GroupStatsAccumulator {
      ...
      void operator()(const std::shared_ptr< const student::Student >& student) const
      {
        struct GradeProcessor {
          ...
          void operator()(const std::pair< date::Date, int >& grade) const
          {
    ...
    struct OtherGroupsGradesCollector {
      ...
      void operator()(const std::pair< std::string, StudentDatabase::Group >& group) const
      {
        ...
        struct StudentProcessor {
          ...
          void operator()(const std::pair< StudentID, std::shared_ptr< student::Student > >& student) const
          {
    ...
    struct IsRiskStudentPredicate {
      double threshold;
      bool operator()(const std::pair< StudentID, std::shared_ptr< student::Student > >& p) const
      {
    ...
    struct IsRiskStudentInGroupPredicate {
      double threshold;
      bool operator()(const std::shared_ptr< const student::Student >& s) const
      {
    ...
    struct GradePairAccumulator {
      double operator()(double sum, const std::pair< StudentID, int >& p) const
      {
        return sum + p.second;
      }
    };
    ...
    struct StudentToFileWriter {
      std::ostream& out;
      void operator()(const std::pair< StudentID, std::shared_ptr< student::Student > >& p) const
      {
        out << *p.second << '\n';
      }
    };
  ...
  StudentDatabase::StudentDatabase(int id_digits)
  {
  ...
  bool StudentDatabase::createGroup(const std::string& groupName)
  {
    return groups.emplace(groupName, Group{}).second;
  }

  bool StudentDatabase::groupExists(const std::string& groupName) const
  {
    return groups.count(groupName) > 0;
  }

  std::pair< bool, StudentID > StudentDatabase::addStudent(const std::string& fullName, const std::string& groupName)
  {
  ...
  bool StudentDatabase::deleteStudent(StudentID id)
  {
  ...
  bool StudentDatabase::moveStudentToGroup(StudentID id, const std::string& newGroupName)
  {
  ...
  std::shared_ptr< const student::Student > StudentDatabase::findStudentById(StudentID id) const
  {
    auto it = students.find(id);
    return (it != students.end()) ? it->second : nullptr;
  }

  std::shared_ptr< student::Student > StudentDatabase::findStudentById(StudentID id)
  {
    auto it = students.find(id);
    return (it != students.end()) ? it->second : nullptr;
  }

  std::vector< StudentID > StudentDatabase::findStudentsByName(const std::string& fullName) const
  {
  ...
  bool StudentDatabase::addGrade(StudentID id, int grade, const date::Date& date)
  {
    auto student_ptr = findStudentById(id);
  ...
  bool StudentDatabase::changeGrade(StudentID id, int newGrade, const date::Date& date)
  {
    auto student_ptr = findStudentById(id);
  ...
  bool StudentDatabase::removeGradesByDate(StudentID id, const date::Date& date)
  {
    auto student_ptr = findStudentById(id);
    if (!student_ptr || !student_ptr->grades_.count(date)) {
      return false;
  ...
  void StudentDatabase::updateStudentAverageGrade(std::shared_ptr< student::Student >& student)
  {
  ...
  bool StudentDatabase::exportGroupForGrading(const std::string& groupName, const std::string& filename) const
  {
  ...
  std::vector< std::shared_ptr< const student::Student > > StudentDatabase::getTopStudents(size_t n) const
  {
  ...
  std::vector< std::shared_ptr< const student::Student > > StudentDatabase::getRiskStudents(double threshold) const
  {
  ...
  std::pair< bool, double > StudentDatabase::getAverageGradeByDate(const date::Date& date) const
  {
